...
//
SSBAnalyzer::SSBAnalyzer(const edm::ParameterSet &iConfig) : effectiveAreas_((iConfig.getParameter<edm::FileInPath>("effAreasConfigFile")).fullPath()),
...
void SSBAnalyzer::GenPar(const edm::Event &iEvent, SSBTreeManager *ssbtreeManager) {
  ...
  if (isPYTHIA) {
	...
	SelectedPar.push_back(GenParIndex);
      ...
      if (itGenPar->status() == 11 && (abs(itGenPar->pdgId()) == 6 || (abs(itGenPar->pdgId()) > 22 && abs(itGenPar->pdgId()) < 26))) {
...
int SSBAnalyzer::IndexLinker(map_i IndexMap, int start_index, int target_depth, int target_index, int target_pdgid, int target_status, bool PrintError, int LoopDepth) {
  if (((start_index == target_index) || (target_index == -999)) &&
    ...
    if (PrintError) {
      ...
      cout << "pdgId : " << AllParInfo[start_index].at(0) << endl
	   << endl;
...
void SSBAnalyzer::FillGenPar(int GenIndex, int FirstMother, int SecondMother, int FirstDaughter, int SecondDaughter, GenParticleCollection::const_iterator itGenParFill, SSBTreeManager *ssbtreeManager) {
